...
def btndelete(self):
    ...
    respquest = messagebox.askyesno(
        title="Delete Group ?",
        message=f"Are you sure you want to delete {self.grpnamecbx.get()}",
        parent=self.dwindow,
    )
    if respquest == True:
        ...
        self.cgouttxtbx.delete("1.0", END)
...
def btnreset(self):
    ...
    self.grpnamebtn.configure(state="normal", text="Set", fg_color="green")
    self.grpnamecbx.configure(state="readonly", fg_color="PaleGreen2")
    self.cgouttxtbx.configure(state="disabled", bg="gray80")
    self.cgouttxtbx.configure(state="normal")
    self.cgouttxtbx.configure(bg="gray80")
    self.cgouttxtbx.delete("1.0", END)
    self.btnreset.configure(state="disabled", fg_color="gray")
...
def grpcbxselc(self):
    ...
    self.grpnamebtn.configure(state="disabled", fg_color="gray", text="Accepted")
    self.grpnamecbx.configure(state="disabled", fg_color="gray80")
    self.cgouttxtbx.configure(state="normal")
    ...
    self.cgouttxtbx.insert(
        INSERT, f"6) Notes/Commands config is : {grpjsondata['commands']}\n"
    )
...
def delgroup(self):
    ...
    self.createlabel = ctk.CTkLabel(
        self.dwindow, text="Delete a Device Group", font=("arial", 22, "bold")
    )
    ...
    self.grpnamebtn = ctk.CTkButton(
        self.dwindow,
        text="Set",
        command=lambda: grpcbxselc(self),
        width=100,
        fg_color="green",
        hover_color="green4",
    )
    ...
    self.cgouttxtbx = scrolledtext.ScrolledText(
        self.dwindow, font="arial 10", height=10, width=25, bg="gray80", wrap=WORD
    )
    self.cgouttxtbx.grid(
        row=4, column=0, sticky=EW, padx=(25, 25), pady=30, columnspan=3
    )
    ...
    self.btnreset = ctk.CTkButton(
        self.dwindow,
        text="Reset",
        command=lambda: btnreset(self),
        fg_color="gray",
        state="disabled",
